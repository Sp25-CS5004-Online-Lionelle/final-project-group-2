...
jobs they have saved, edit them, and delete them (Download will be located on the pane). When clients views a job, 
we will also conduct an API call that will grab related skill to the job.
}
}
...
note ["This will implement the logic for review saved jobs."]
}

class FindJobController {
note ["This will set the main pane and implement the two tabs."]
}
...
class JobSorts {
note ["Sorting Logic for jobs"]
}

class DataFormatters {
note:["Port over from HW 9 => helper functions for saving data"]
}

class NetUtils {
note:["Port over from HW 9 => helper functions for get request"]
}
}
```
